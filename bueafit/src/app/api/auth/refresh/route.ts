...
export async function POST() {
  ...
  console.log('Next.js cookies:', cookieStore.getAll().map(c => `${c.name}=${c.value}`).join('; '))

  const refreshToken = cookieStore.get('refresh_token')?.value
  console.log('[auth/refresh] refresh_token:', refreshToken)
  if (!refreshToken) {
  ...
  // 3) 디버그: 백엔드 응답 확인
  console.log('[auth/refresh] status:', backendRes.status)
  const text = await backendRes.text()
  console.log('[auth/refresh] body:', text)
